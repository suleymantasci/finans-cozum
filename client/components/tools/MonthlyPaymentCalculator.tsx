 ...
 * KKDF ve BSMV dahil hesaplama
 */
export default function MonthlyPaymentCalculator({ config }: ToolComponentProps) {
  ...
  const calculate = () => {
    ...
    const totalCost = totalInterest + totalKkdf + totalBsmv
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Aylık Taksit Hesaplama</CardTitle>
        <CardDescription>Kredi aylık taksit tutarını hesaplayın (KKDF ve BSMV dahil)</CardDescription>
      </CardHeader>
          ...
          <div className="mt-4 space-y-2 rounded-lg bg-muted p-4">
            ...
            {creditType !== 'none' && (
              <div className="text-xs text-muted-foreground">
                (Vergisiz: {result.monthlyPayment.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺)
              </div>
            )}
